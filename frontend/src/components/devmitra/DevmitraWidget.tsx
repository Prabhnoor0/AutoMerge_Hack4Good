...
import { api } from "@/lib/api";
import { useDevmitra } from "@/store/DevmitraContext";
import { DevmitraAvatar } from "./DevmitraAvatar";
...
export function DevmitraWidget() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        const status = (await api.devmitraContextStatus()) as ContextStatus;
        setContextStatus(status);
      } catch (e) {
        console.error("Context status fetch failed:", e);
      }
    };
  ...
  const handleSend = async (text: string = input) => {
    ...
    try {
      ...
      const res = await api.devmitraChat(text, context, currentSessionId);
  ...
  const hasContext = !!(
    ...
    context.repoUrl ||
    context.logs ||
    contextStatus?.has_context
  );

  const contextLabel =
    contextStatus?.primary_label ||
    (hasContext
      ? `Context: ${context.filename || context.repoUrl || "Code snippet"}`
    ...
    <>
      {!isOpen && (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
          ...
          >
            <DevmitraAvatar currentState={loading ? "thinking" : "idle"} />
          </div>
          ...
          <motion.button
            ...
            className="flex items-center gap-2 px-4 py-3 rounded-full text-white shadow-2xl"
            style={{
              background: "linear-gradient(135deg, #4f8ef7, #8b5cf6)",
              boxShadow: "0 8px 32px rgba(79, 142, 247, 0.4)",
            }}
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            >
              <div className="flex items-center gap-2">
                <div className="scale-[0.6] -ml-4 -mr-2">
                  <DevmitraAvatar currentState={loading ? "thinking" : "idle"} />
                </div>
                <div>
            ...
            <div
              className="px-4 py-2 border-b flex items-center gap-2 bg-[var(--bg-elevated)]"
              style={{ borderColor: "var(--border)" }}
            >
              {hasContext ? (
                <div
              ...
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-70">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-1 max-w-[250px]">
                      {hasContext
                        ? "I see you have code loaded. Ask me to explain it or find bugs!"
                        : "Open the Studio or Workspace to load code context."}
                    </p>
                  </div>
            ...
            <div
              className="p-3 bg-[var(--bg-secondary)] border-t"
              style={{ borderColor: "var(--border)" }}
            >
              <form
                onSubmit={(e) => {
