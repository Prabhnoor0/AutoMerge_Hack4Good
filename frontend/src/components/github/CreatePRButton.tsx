...
import { api } from "@/lib/api";
import { CombatFixOverlay } from "../dashboard/CombatFixOverlay";
...
export function CreatePRButton({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (token) {
        res = await api.createRepoPR({
          job_id: jobId,
          token,
          base_branch: baseBranch || "main",
        });
      } else {
        res = await api.githubCreatePR(jobId, baseBranch);
      }
  ...
  if (result && !result.success) {
    return (
      ...
      >
        <AlertCircle size={14} />
        <span className="flex-1">{result.error}</span>
        <button onClick={() => setResult(null)} className="ml-auto underline">
          Retry
        </button>
      </div>
    );
