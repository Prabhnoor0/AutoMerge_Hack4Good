...
import { EmptyState } from "@/components/dashboard/EmptyState";
import { useJobs, useJobDetail } from "@/hooks/useJobs";
import { CodeHealthPlanet } from "@/components/visual/CodeHealthPlanet/CodeHealthPlanet";
import { TimeTravelView } from "@/components/debug/TimeTravelView/TimeTravelView";
...
export default function DashboardPage() {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: "var(--bg-primary)" }}>
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="p-1.5 hover:bg-white/10 rounded text-cyan-400/70 hover:text-cyan-400 transition"
          ...
          <AnimatePresence mode="wait">
            {selectedJob ? (
              ...
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex-1 flex flex-col items-center justify-center min-h-[500px] px-6 text-center"
              >
                <CodeHealthPlanet jobs={jobs} />
                <h3 className="text-lg font-bold text-white mt-8 tracking-widest uppercase">
                  System Status Monitor
                </h3>
                <p className="text-sm mt-1 max-w-md" style={{ color: "var(--text-muted)" }}>
                  Select an anomaly trace to investigate
                </p>
      ...
      <AnimatePresence>
        {timeTravelOpen && (
          <TimeTravelView jobs={jobs} onClose={() => setTimeTravelOpen(false)} />
        )}
      </AnimatePresence>
    </div>
  );
