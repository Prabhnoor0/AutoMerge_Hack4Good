...
const STATUS_EMOJI: Record<string, string> = {
  success: "✅", failure: "❌", error: "⚠️", timeout: "⏰", running: "⏳", pending: "…",
};
      ...
      <div className="w-72 flex-shrink-0 border-r flex flex-col" style={{ borderColor: "var(--border)", background: "var(--bg-secondary)" }}>
        ...
        <div className="p-4 border-b" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg,#06b6d4,#8b5cf6)", boxShadow: "0 4px 16px rgba(6,182,212,0.3)" }}>
        ...
        <div className="flex border-b" style={{ borderColor: "var(--border)" }}>
          {(["editor", "history"] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className="flex-1 py-2.5 text-[11px] font-semibold uppercase tracking-wider transition-all" style={{ color: activeTab === tab ? "var(--accent-blue)" : "var(--text-muted)", borderBottom: activeTab === tab ? "2px solid var(--accent-blue)" : "2px solid transparent" }}>
              {tab === "editor" ? "⚡ Editor" : `📋 History (${history.length})`}
            </button>
          ))}
        </div>
        ...
        {activeTab === "editor" && (
          <div className="flex-1 overflow-auto p-4 space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(["run", "test"] as const).map(m => (
                  <button key={m} onClick={() => setMode(m)} className="flex-1 py-2 rounded-lg text-xs font-semibold transition-all" style={{ background: mode === m ? "var(--accent-blue)15" : "var(--bg-elevated)", color: mode === m ? "var(--accent-blue)" : "var(--text-muted)", border: `1px solid ${mode === m ? "var(--accent-blue)" : "var(--border)"}` }}>
                    {m === "run" ? "▶ Run" : "🧪 Test"}
                  </button>
                ))}
              </div>
            ...
            {/* Config Toggle */}
            <button onClick={() => setShowConfig(!showConfig)} className="w-full text-left text-[10px] font-semibold uppercase tracking-wider py-1" style={{ color: "var(--text-muted)" }}>
            ...
            <div className="p-3 rounded-lg" style={{ background: "rgba(6,182,212,0.08)", border: "1px solid rgba(6,182,212,0.15)" }}>
              <p className="text-[10px] font-semibold mb-1" style={{ color: "#06b6d4" }}>🔒 Container Isolated</p>
            ...
            {history.map(h => (
              <button key={h.run_id} onClick={() => viewHistoryRun(h.run_id)} className="w-full text-left p-3 rounded-lg transition-all hover:opacity-80" style={{ background: "var(--bg-elevated)" }}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-semibold" style={{ color: STATUS_COLORS[h.status] || "#6b7280" }}>{STATUS_EMOJI[h.status]} {h.status}</span>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <button onClick={e => { e.stopPropagation(); deleteRun(h.run_id); }} className="ml-auto text-[9px]" style={{ color: "var(--text-muted)" }}>🗑</button>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden border-r" style={{ borderColor: "var(--border)" }}>
            <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: "var(--border)", background: "var(--bg-secondary)" }}>
              <h3 className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--accent-blue)" }}>
                {mode === "test" ? "🧪 Code + Tests" : "▶ Code"}
              </h3>
            ...
            <div className="p-3 border-t" style={{ borderColor: "var(--border)", background: "var(--bg-secondary)" }}>
              <button onClick={handleRun} disabled={running || !code.trim()} className="w-full py-2.5 rounded-xl text-sm font-bold transition-all" style={{ background: running ? "var(--bg-elevated)" : "linear-gradient(135deg,#06b6d4,#8b5cf6)", color: "white", opacity: (!code.trim() || running) ? 0.5 : 1 }}>
                {running ? "⏳ Executing in Container..." : mode === "test" ? "🧪 Run Tests" : "▶ Execute Code"}
              </button>
            </div>
          </div>
              ...
              {result && !running && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: `${STATUS_COLORS[result.status] || "#6b7280"}10`, border: `1px solid ${STATUS_COLORS[result.status] || "#6b7280"}30` }}>
                    <div className="flex items-center gap-2">
                      <span className="text-lg">{STATUS_EMOJI[result.status]}</span>
                      <div>
