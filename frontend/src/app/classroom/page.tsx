...
import { api } from "@/lib/api";
import type { ClassroomReport, ClassroomSummary, ClassroomResource } from "@/lib/types";
import { BugChallengeOverlay } from "@/components/classroom/GameLayer/BugChallengeOverlay";
...
export default function ClassroomPage() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [challengeReport, setChallengeReport] = useState<ClassroomReport | null>(null);
  ...
  const handleUpdate = async (
    id: string,
    update: { status?: string; revision_done?: boolean }
  ) => {
    try {
      const updated = await api.classroomUpdateReport(id, update);
      setReports((prev) => prev.map((r) => (r.id === id ? updated : r)));
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
              Analyzing your debugging history...
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: "var(--bg-primary)" }}>
        ...
        {summary && (
          ...
          >
            ...
            <StatCard
              ...
              label="Revised"
              value={summary.revision_done_count}
            ...
            <AnimatePresence mode="popLayout">
              {filtered.map((report, i) => (
                <ReportCard
                  ...
                  deleteConfirm={deleteConfirm}
                  setDeleteConfirm={setDeleteConfirm}
                  onPlayChallenge={(r) => setChallengeReport(r)}
                />
              ))}
            </AnimatePresence>
          </div>
      ...
      <AnimatePresence>
        {challengeReport && (
          <BugChallengeOverlay report={challengeReport} onClose={() => setChallengeReport(null)} />
        )}
      </AnimatePresence>
    </div>
  );
    ...
    >
      ...
      {hasAnyReports ? (
        ...
        <>
          ...
          <p className="text-xs mt-2 max-w-sm leading-relaxed" style={{ color: "var(--text-muted)" }}>
            Classroom generates reports by analyzing your debugging history. Run some debugging jobs first,
            then click <strong>Refresh Reports</strong> to let AutoMerge identify your recurring weakness areas.
          </p>
          <button
            onClick={onRefresh}
            disabled={refreshing}
...
function ReportCard({
  ...
  deleteConfirm: string | null;
  setDeleteConfirm: (id: string | null) => void;
  onPlayChallenge: (report: ClassroomReport) => void;
}) {
  const isCompleted = report.status === "completed";
  const severityColor =
    report.severity_score >= 0.7
      ? "var(--accent-red)"
      : report.severity_score >= 0.4
        ? "var(--accent-amber)"
        : "var(--accent-cyan)";

  const severityLabel =
    report.severity_score >= 0.7 ? "High" : report.severity_score >= 0.4 ? "Medium" : "Low";
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2 text-[10px] flex-wrap" style={{ color: "var(--text-muted)" }}>
            <span className="flex items-center gap-1">
              <Target size={10} style={{ color: severityColor }} />
              {severityLabel} severity
            </span>
        ...
        {expanded && (
          ...
          >
            <div className="px-5 pb-5 space-y-4 border-t" style={{ borderColor: "var(--border-subtle)" }}>
              ...
              <div className="flex items-center gap-2 pt-2 flex-wrap">
                <button
                  onClick={() => onPlayChallenge(report)}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all shadow-[0_0_15px_rgba(0,240,255,0.2)] hover:shadow-[0_0_25px_rgba(0,240,255,0.4)]"
                  style={{ background: "linear-gradient(135deg, #00f0ff, #0080ff)", color: "black" }}
                >
                  <PlayCircle size={14} />
                  SIMULATE BREACH (PLAY)
                </button>
